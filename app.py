...
# Maximum allowed file size for uploads (MB)
MAX_FILE_SIZE_MB = 50
...
uploaded_files = st.file_uploader(
    "Upload Garmin R10 CSV files", type="csv", accept_multiple_files=True
)
        ...
        if not dfs:
            ...
            if "Carry" in df_all.columns:
                if df_all["Carry"].dtype == "object":
                    st.warning(
                        "Carry column contains non-numeric data. Please check your CSV."
                    )
                else:
