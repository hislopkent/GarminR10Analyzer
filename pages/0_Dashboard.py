...
if df_all is None or df_all.empty:
    ...
    total_sessions = df_all["Session"].nunique()
    total_clubs = df_all["Club"].nunique()

    metric_cols = st.columns(3)
    metric_cols[0].metric("Total Shots", total_shots)
    metric_cols[1].metric("Sessions Loaded", total_sessions)
    metric_cols[2].metric("Clubs Used", total_clubs)
    ...
    def highlight_smash(v):
        if isinstance(v, (int, float)) and v < 1.3:
            return "background-color: red"
        return ""

    styled_table = grouped.style.applymap(highlight_smash, subset=["Smash Factor"])
    ...
    if api_key and st.button("Generate AI Insights"):
        try:
            ...
            prompt = f"Based on these golf shot averages per club (mean, median, std for Carry, Backspin, Sidespin, Total, Smash Factor, Apex Height, Launch Angle, Attack Angle):\n{grouped.to_string()}\nProvide suggestions, comments, and recommendations for improving performance. Focus on general aspects aspects like fat/thin shots (low Smash Factor, extreme Launch Angle/Backspin), outliers, consistency, and typical golf benchmarks (e.g., driver carry >200 yards, irons Smash Factor >1.3)."
